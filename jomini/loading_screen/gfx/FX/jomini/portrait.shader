...
VertexShader = {
	...
	Code
	[[
		VS_OUTPUT_PDXMESH PdxMeshVertexBillboardShader( VS_INPUT_PDXMESH Input, uint JointsInstanceIndex, float4x4 WorldMatrix )
		{
			...
			float3x3 Rotation = CastTo3x3(World);
			Out.Normal = normalize( mul( Rotation, Normal ) );
			Out.Tangent = normalize( mul( Rotation, Tangent ) );
			...
			Out.Position = FixProjectionAndMul( ProjectionMatrix, Out.Position );
		...
		VS_OUTPUT_PDXMESH PdxMeshVertexBillboardShaderStandard( VS_INPUT_PDXMESHSTANDARD Input )
		{
			return PdxMeshVertexBillboardShader( PdxMeshConvertInput( Input ), Input.InstanceIndices.x, PdxMeshGetWorldMatrix( Input.InstanceIndices.y ) );
		}
	...
	MainCode VS_billboard
	{
		...
		[[
			PDX_MAIN
			{
				VS_OUTPUT_PDXMESHPORTRAIT Out = ConvertOutput( PdxMeshVertexBillboardShaderStandard( Input ) );
				Out.InstanceIndex = Input.InstanceIndices.y;
				return Out;
			}
		]]
...
Effect portrait_attachment_alpha_to_coverage_billboarded
{
	VertexShader = "VS_billboard"
	PixelShader = "PS_attachment"
	BlendState = "alpha_to_coverage"
	Defines = { "PDX_MESH_BLENDSHAPES" }
}
